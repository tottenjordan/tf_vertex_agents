...
https://source.corp.google.com/piper///depot/google3/research/sir/rl_research/proxy_rewards/movielens/movie_lens_simulator.py
"""
import collections
import functools
...
import attr
from fairness_gym import core
from fairness_gym.environments.recommenders import recsim_samplers
...
from google3.research.sir.rl_research.proxy_rewards.movielens import utils
...
class Movie(document.AbstractDocument):
  ...
  def __init__(self, doc_id: int, title: str, genres: List[int],
               vec: List[float]):
...
class User(user.AbstractUserState):
  ...
  def score_document(self, doc: Movie, lambda_diversity: float = 3.) -> float:
  ...
  def _update_affinity_vector(self, doc: Movie):
  ...
  def _update_genre_history(self, doc: Movie):
    for genre in doc.genres:
      self.genre_history[genre] = 1.

  def update_state(self, doc: Movie, response: Response):
...
class UserModel(user.AbstractUserModel):
  ...
  def simulate_response(self, documents: List[Movie]) -> Response:
...
def create_gym_environment(env_config: EnvConfig):
